  		 	        ...
  		 	        <tbody>
				        ...
				        ?>
							<tr>
								...
								<td>
							    	<a class="icon-edit hovertip" data-toggle="modal" data-original-title="<?php _e("Edit Client?", bookings_plus ); ?>" data-placement="top" href="#EditCustomer" onclick="editCustomers(<?php echo $customers[$flag]->CustomerId;?>);"></a>&nbsp;&nbsp;
							      	<a class="icon-envelope hovertip" data-toggle="modal" data-original-title="<?php _e("Email Client?", bookings_plus ); ?>" data-placement="top" href="#customerEmail" onclick="emailCustomer(<?php echo $customers[$flag]->CustomerId;?>);" data-toggle="modal"></a>&nbsp;&nbsp;
							      		<?php
							      	if($paypalEnable == 0)
									{
										?>
										<a class="icon-shopping-cart hovertip" data-toggle="modal" data-original-title="<?php _e("Payment Details", bookings_plus ); ?>" data-placement="top" href="#paypalDetails" onclick="customerPaypalBookingdetails(<?php echo $customers[$flag]->CustomerId;?>)"></a>&nbsp;&nbsp;
										<?php
									}
									?>								                									                		
							       	<a class="icon-calendar hovertip" data-toggle="modal" data-original-title="<?php _e("Booking Details", bookings_plus ); ?>" data-placement="top"  href="#manageBookings" onclick="customerBookingdetails(<?php echo $customers[$flag]->CustomerId;?>)" ></a>&nbsp;&nbsp;							                		
							       	<a class="icon-trash hovertip"  data-toggle="modal" data-original-title="<?php _e("Delete Client", bookings_plus ); ?>" data-placement="top"   onclick="deleteCustomer(<?php echo $customers[$flag]->CustomerId;?>)"></a>							                									                		
							    </td>
						   </tr>
					       <?php	
				        }		
...
<div id="EditCustomer" class="modal hide fade" role="dialog" aria-hidden="true">
...
<div id="manageBookings" class="modal hide fade" role="dialog" aria-hidden="true">
	<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">x</button>		
...
<div id="paypalDetails" class="modal hide fade" role="dialog" aria-hidden="true">
	<div class="modal-header">	
	    <h5><?php _e( "Payment Details", bookings_plus ); ?> -  <span id="CustomerPaymentSchedule"></span></h5>	    	
    </div>	
            ...
            <div class="body" style="padding:0px;">
				<div class="table-overflow">
					<table class="table table-striped" id="data-table-paypal-bookings">
						<thead>
								<tr>
									<th style="width:10%"><?php _e( "Service", bookings_plus ); ?></th>
									<th style="width:10%"><?php _e( "Cost", bookings_plus ); ?></th>
									<th style="width:10%"><?php _e( "Date", bookings_plus ); ?></th>
									<th style="width:10%"><?php _e( "Time Slot", bookings_plus ); ?></th>
									<th style="width:10%"><?php _e( "Payment Status", bookings_plus ); ?></th>
									<th style="width:10%"><?php _e( "Trns. ID", bookings_plus ); ?></th>										
									<th style="width:10%"><?php _e( "Trns. Date", bookings_plus ); ?></th>
								
								</tr>
						</thead>
						<tbody id="bindCustomerPaypalBookings"></tbody>
					</table>
				</div>
	    	</div>
	   </div>
...
<div id="customerEmail" class="modal hide fade" role="dialog" aria-hidden="true">
	<div class="modal-header">
		 <button type="button" class="close" data-dismiss="modal" aria-hidden="true">x</button>	
		...
		<div class="body">
	        ...
	        <div class="block well" style="margin-top:10px;">
	   			<div class="row-fluid form-horizontal">
    				...
    				<?php 
    				  	$content ="";
    				 	wp_editor($content, $id = 'uxFrmCustomerEmailTemplate', $prev_id = 'title', $media_buttons = true, $tab_index = 2); 
    				?>
					</div>                      
	            </div>
	        </div>
...
<div id="customerComments" class="modal hide fade" role="dialog" aria-hidden="true">
	<div class="modal-header">	
...
<div id="bookingLink" class="modal hide fade" role="dialog" aria-hidden="true">
	
	<?php include_once 'bookingCalendarByCustomer.php' ?>	
</div>
	...
	({
		...
		function exportDataCustomer()
		{
			...
			({
				type: "POST",
				data: "",
				url:  uri+"/exportData.php",
				success: function(data) 
				{  
			    	window.location.href = uri+"/exportData.xlsx";
			    	bootbox.alert("<?php _e("Thank you for exporting your Customers Data.", bookings_plus ); ?>");
			    }
			});
		}
		...
		function customerPaypalBookingdetails(id)
		{
			...
			({
				type: "POST",
				data: "customerId="+id+"&target=customerPaypalBooking&action=getAjaxExecuted",
				url:  ajaxurl,
			 	success: function(data) 
				{  
					jQuery('#bindCustomerPaypalBookings').empty();
			    	jQuery('#bindCustomerPaypalBookings').html(data);
